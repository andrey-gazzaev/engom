...
import { createObserverSpy } from '@engom/common/testing/utils';
...
describe('catchValidationError(x)', () => {
	...
	describe('with form group provided', () => {
		...
		function testValidationData(validationData: TestValidationData): void {
			...
			throwError(() => passedError)
				.pipe(catchValidationData(formGroupMock))
				.subscribe(observer);
		...
		function expectFormToHave(validationData: TestValidationData): void {
			const validationDataToError = (data: TestValidationData[keyof TestValidationData]): ValidationErrors | null =>
				data != null ? AppValidators.buildAppError(data.toString()) : null;
			...
			expect(formGroupMock.controls.childValue1.errors).toEqual(validationDataToError(validationData.childValue1));
			expect(formGroupMock.controls.childValue2.errors).toEqual(validationDataToError(validationData.childValue2));
			...
			if (typeof validationData.childValueObject === 'string') {
				...
				expect(formGroupMock.controls.childValueObject.controls.nestedChildValue1.errors).toEqual(
					validationDataToError(
						(
							validationData.childValueObject as EntityValidationErrors<
							ExtractFormValue<TestForm['controls']['childValueObject']>
							>
						)?.nestedChildValue1,
					),
				);
		...
		describe('when got validation data', () => {
			it('sets errors for the child elements', () =>
				testValidationData({
					childValue1: 'Test error',
					childValue2: 'Test error 2',
				}));

			it('sets errors for nested child elements', () =>
				testValidationData({
					childValueObject: {
			...
			it('sets an error for a nested form group', () =>
				testValidationData({
					childValueObject: 'error',
				}));

			it('sets errors for both nested and first-level child elements', () =>
				testValidationData({
					childValue1: 'Test error',
					childValueObject: {
						nestedChildValue1: 'Test error',
	...
	describe('with subject provided', () => {
		...
		describe('when got AppValidationError', () => {
			it('emits a validation data to subject', () => {
				...
				throwError(() => error)
					.pipe(catchValidationData(spySubject))
					.subscribe(observer);
